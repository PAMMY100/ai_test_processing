...
const About = ({ onOpen }) => {
  return (
    <div className='w-[90%] h-[700px] md:h-[980px] rounded-2xl text-white border-2 border-blue-100 bg-gradient-to-r from-slate-700 via-slate-800 to-slate-900 flex flex-col gap-6 md:gap-14 justify-center items-center p-10 m-auto font-serif'>
      <h1 className='sm:text-xl text-2xl font-bold mb-4 md:text-4xl animate-bounce bg-gradient-to-r from-blue-100 via-blue-300 to-blue-700 bg-clip-text text-transparent'>AI Text Processing</h1>
      <section className='text-xl md:text-2xl flex flex-col gap-5 text-center'>
        <p>⚠️ Use the app in Chrome Browser</p>
        <p>🖊️ Enter text for processing</p>
        <p>🔍 AI will automatically detect the text's language</p>
        <p>🔁 Translate to any available language</p>
        <p>📄 Summarize text longer than 150 characters</p>
      </section>
      <button 
        onClick={onOpen} 
        className='mt-3 h-16 w-[200px] p-3 bg-white text-blue-800 font-bold border-slate-500 rounded-lg hover:bg-slate-600 hover:text-white transition duration-700 ease-in-out md:w-[300px] md:text-2xl'>
        Get Started
      </button>
    </div>
  )
